...
def main():
    """The main function"""
    df = pd.read_csv("drug_bank.csv").dropna(subset=["SMILES"]).reset_index(drop=True)
    enc = MHFPEncoder()
    ...
    # f = Faerun(view="front", coords=False, background_color="#FFFFFF")  # #FFFFFF is the hex code for white
